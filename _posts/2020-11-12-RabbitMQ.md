...
1. 简单队列 
一个生产者对应一个消费者。
![Simple Queue](https://www.rabbitmq.com/img/tutorials/python-one.png)
2. 工作队列
...
3. 发布/订阅
  fanout
  ![Publish/Subscribe](https://www.rabbitmq.com/img/tutorials/direct-exchange.png)
4. RPC
...
- 注意事项 如果一个消费者在处理消息出现了网络不稳定、服务器异常等现象，那么就不会有ACK反馈，RabbitMQ会认为这个消息没有正常消费，会将消息重新放入队列中。如果忘记了ACK，那么后果很严重。当Consumer退出时候，Message会一直重新分发。然后RabbitMQ会占用越来越多的内容，由于RabbitMQ会长时间运行，因此这个"内存泄漏"是致命的。 
#### prefetch
...
- 异步处理
  ...
  ![img](https://img-blog.csdn.net/20170209150824008?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvd2hvYW1peWFuZw==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)

- 应用解耦

  引入消息队列解耦的同时，还能在消费者系统出现故障的时候进行对旧数据的恢复。

  ![img](https://img-blog.csdn.net/20170209152116530?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvd2hvYW1peWFuZw==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)

- 流量削峰

  用户的请求,服务器收到之后,首先写入消息队列,加入消息队列长度超过最大值,则直接抛弃用户请求或跳转到错误页面.

  ![img](https://img-blog.csdn.net/20170209161124911?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvd2hvYW1peWFuZw==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)
...
| 功能特性   | 完备w    | 完备 高性能 低延时 | 较为完善   | 较为简单 只支持基础的MQ功能 |
